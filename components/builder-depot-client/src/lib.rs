...
pub struct OriginChannel {
    pub ident: OriginChannelIdent,
    pub name: String,
    pub origin: String,
}

impl Into<originsrv::OriginChannel> for OriginChannel {
    fn into(self) -> originsrv::OriginChannel {
        let mut out = originsrv::OriginChannel::new();
        out.set_name(self.name);
        out
    }
}
